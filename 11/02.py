...
class planet():
    ...
    def calculate(self):
        ...
        for i in range(1,nsteps):
            self.vx.append(self.vx[i-1]-((self.m1+self.m2)/self.m2)*(4*(math.pi)**2)*(self.x[i-1]/(self.r[i-1])**3)*self.dt)
            self.x.append(self.x[i-1]+self.vx[i]*self.dt)
            self.vy.append(self.vy[i-1]-((self.m1+self.m2)/self.m2)*(4*(math.pi)**2)*(self.y[i-1]/(self.r[i-1])**3)*self.dt)
...
A=planet(1,0,0,2*math.pi,0.002,10,1000,1)
A.calculate()
A.plot("blue")
pl.show()
